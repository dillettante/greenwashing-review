...
2. 각 주장에 대해 **직접 근거와 사례**를 확인한다.
   ...
   - **법원 판례 시맨틱 검색(court-precedents MCP, 2026-07-22 추가)**: 판례공보·판결공보 축적분을
     ...
     표시광고 사건의 상당수가 시정명령·과징금 **처분취소 소송**(두 사건)이라 이 DB 주력과 겹친다
     (예: 대법원 2026. 5. 14. 선고 2023두50080 — 시정명령의 내용적 한계). 격주 자동 갱신·속보 포함
     **최신 판례**가 강점. 단 2026년~ 축적분이므로 그 이전 판례는 아래 루프로 보완.
   - **심결례·과거 판례는 5단계 루프**(SKILL.md ②-2와 동일). 법제처 판례 API 직접검색이 잘 안 되는
     이유는 관련도 랭킹이 없고 선고일 역순으로만 정렬되기 때문이다. **의결서를 다리로 쓴다**:
...
### 2-A. 문체·표현 규칙 (필수) — 산출물은 **의뢰인이 읽는 문서**다

`2-evaluation.json`의 서술 필드는 그대로 보고서 3종(.md·.docx·.html)에 렌더된다. 따라서 **작성 단계부터**
대고객 문서의 문체로 쓴다([[CLAUDE]] §3: 의견서·서면 → 격식체).

- **격식체(합쇼체)로 쓴다.** "…한다/이다/없다" → "…합니다/입니다/없습니다".
  명사형 종결("…소재.", "…확산.", "…않음")도 문장으로 편다 — 개조식 메모가 아니라 의견서다.
- **내부 용어를 노출하지 않는다.** 정규식·세션·LLM·MCP·앵커·트리아지·shortlist·파일명
  (`3-redline.md` 등)은 의뢰인의 언어가 아니다. → "문안 수정 권고", "원문 대조 확인" 등으로 쓴다.
- **판단은 분명히, 단정은 신중히.** 확인된 것과 추정을 구분하고, 미확인은 `confirm_needed`로 넘긴다.
- **원문은 불변.** `quote`(검토 대상 원문)·`precedents[].holding`(판례 원문)·법령 문구는
  문체 전환 대상이 아니다. 인용은 원문 그대로 둔다.
- **검증 이력을 서술문에 섞지 않는다.** `[2026-07-21 국가법령정보로 현행 확인]`,
  `[의결서 원문 범위 — 변호사 확인]` 같은 **작업 기록용 대괄호 주석을 본문·`basis`에 넣지 않는다.**
  확인 사실은 남기지 말고, 확인이 **덜 된 사항**만 `confirm_needed`로 옮긴다.
- **`precedents[].status`는 법적 절차 단계만 적는다.** 인용 가치에 직결되는 정보이기 때문이다.
  - 쓸 것: `확정` / `공정위 의결(2025.4.14)` / `피심인 위법성 수락` / `ASA 재결(비교법)` / `헌법재판소 결정`
  - 쓰지 말 것: `로컬 아카이브 확인`, `국가법령정보 원문 확인`, `변호사 확인`(수집 경로·작업 지시)
  - 확정 여부를 확인하지 못했으면 `사법 확정 여부 미확인`으로 **중립 표기**한다([[CLAUDE]] §2-1).
  - HTML 렌더러(`_public_status`)가 방어적으로 한 번 더 정제하지만, **작성 단계에서 지키는 것이 원칙**이다.
- 공격/방어 어느 목적이든 **감정적·단정적 수사**를 피한다("가장 다투기 좋은 표적" 같은 표현 금지).

> 이미 보고체로 작성된 기존 산출물은 종결어미 일괄 치환으로 완전히 교정되지 않는다
> (명사형 종결·내부 참조가 남는다). 재평가 시 이 규칙으로 다시 쓴다.
