...
**문체·표현(필수, SOP §2-A).** `2-evaluation.json`의 서술은 그대로 의뢰인이 읽는 보고서가 된다.
**격식체(합쇼체)**로 쓰고 명사형 종결("…소재.", "…않음")도 문장으로 편다. **내부 용어를 노출하지 않는다**
(정규식·세션·LLM·MCP·앵커·트리아지·파일명 `3-redline.md` 등 → "문안 수정 권고"·"원문 대조 확인"으로).
단정적·감정적 수사는 피한다. **단, `quote`·판례 `holding`·법령 문구는 원문 그대로 둔다.**
